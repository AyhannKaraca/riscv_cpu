...
test:
	cd test && \
	/opt/riscv/bin/riscv32-unknown-elf-as -march=rv32i_zbb -mabi=ilp32 -o $(TEST_FILE).o $(TEST_FILE).s && \
	...
	xxd -p -c 4 $(TEST_FILE).bin | sed 's/\(..\)\(..\)\(..\)\(..\)/\4\3\2\1/' > $(TEST_FILE).hex && \
	/opt/riscv/bin/spike -d --debug-cmd=spike.script -l --log-commits -m0x7ffff000:0x20000000 --isa=rv32i_zbb --log=$(TEST_FILE)_golden.log $(TEST_FILE).elf && \
